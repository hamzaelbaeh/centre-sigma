...
@section('content')
@php $fmt = fn($n) => money_ltr($n); @endphp
<div class="page-head">
 <div><h1>{{ __('Tableau de bord') }}</h1><div class="sub">{{ format_date_latin(null, 'l d/m/Y') }} · {{ format_date_latin(null, 'F Y') }}</div></div>
 <div class="actions">
...
<tr><td>{{ $m['label'] }}</td><td><span class="ltr-num">{{ number_format($m['recettes'],2,',',' ') }}</span></td><td><span class="ltr-num">{{ number_format($m['depenses'],2,',',' ') }}</span></td><td><span class="ltr-num">{{ number_format($m['salaires'],2,',',' ') }}</span></td></tr>
@endforeach
</tbody></table></div>
...
<div>
<div class="card" style="margin-bottom:14px"><h3>{{ __('Enseignants à payer') }} — {{ format_date_latin(null, 'F Y') }}</h3><div class="empty"><div class="ok">OK</div>{{ __('Aucune paie en attente.') }} <a href="{{ route('payroll_teachers.index') }}">{{ __('Générer la paie') }}</a></div></div>
...
<div class="card" style="margin-top:14px;margin-bottom:14px">
 ...
 <div id="slot-tab-subjects" class="table-wrap">
  <table class="data">
   ...
   <tr>
    <td>{{ $row['name'] }}</td>
    <td><span class="ltr-num">{{ $row['sessions'] }}</span></td>
    <td><span class="ltr-num">{{ number_format($row['hours'], 2, ',', ' ') }}</span></td>
    <td><span class="ltr-num">{{ $row['monthly_sessions'] }}</span></td>
    <td><span class="ltr-num">{{ number_format($row['monthly_hours'], 2, ',', ' ') }}</span></td>
   </tr>
   @empty
